...
public:
    PNDMImpl(){}
    PNDMImpl(po::variables_map &vm, torch::Device device);
    std::tuple<torch::Tensor, torch::Tensor> add_noise(torch::Tensor x_0, torch::Tensor t);
    torch::Tensor transfer(torch::Tensor x_t, torch::Tensor eps, torch::Tensor alpha_bar, torch::Tensor alpha_bar_prev);
    torch::Tensor denoise(torch::Tensor x_t, torch::Tensor t, torch::Tensor t_prev, std::vector<torch::Tensor> &eps_history);
