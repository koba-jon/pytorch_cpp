...
void test(po::variables_map &vm, torch::Device &device, PNDM &model, std::vector<transforms_Compose> &transform);
void synth(po::variables_map &vm, torch::Device &device, PNDM &model);
void sample(po::variables_map &vm, torch::Device &device, PNDM &model);
...
int main(int argc, const char *argv[]){
    ...
    // (5) Define Network
    PNDM pndm(vm, device); pndm->to(device);
    PNDM pndm_aux(vm, device); pndm_aux->to(device);
    ...
    Set_Model_Params(vm, pndm, "PNDM");
    Set_Model_Params(vm, pndm_aux, "PNDM_Auxiliary");

    // (8.1) Training Phase
    if (vm["train"].as<bool>()){
        Set_Options(vm, argc, argv, args, "train");
        train(vm, device, pndm, pndm_aux, transform);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, pndm, transform);
    }

    // (8.3) Synthesis Phase
    if (vm["synth"].as<bool>()){
        Set_Options(vm, argc, argv, args, "synth");
        synth(vm, device, pndm);
    }

    // (8.4) Sampling Phase
    if (vm["sample"].as<bool>()){
        Set_Options(vm, argc, argv, args, "sample");
        sample(vm, device, pndm);
    }
