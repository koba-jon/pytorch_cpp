...
void synth(po::variables_map &vm, torch::Device &device, NormalizingFlow &model){
    ...
    outputs = model->inverse(z1).view({1, (long int)vm["nc"].as<size_t>(), (long int)vm["size"].as<size_t>(), (long int)vm["size"].as<size_t>()});
    for (size_t i = 1; i < count; i++){
        z = z1 * (count - i - 1) / (count - 1) + z2 * i / (count - 1);
        output = model->inverse(z).view({1, (long int)vm["nc"].as<size_t>(), (long int)vm["size"].as<size_t>(), (long int)vm["size"].as<size_t>()});
