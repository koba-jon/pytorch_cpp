...
void train1(po::variables_map &vm, torch::Device &device, VQVAE2 &model, std::vector<transforms_Compose> &transform){
    ...
    if (vm["train1_load_epoch"].as<std::string>() == ""){
        ...
        path = checkpoint_dir + "/models/epoch_" + vm["train1_load_epoch"].as<std::string>() + "_vqvae2.pth";  torch::load(model, path, device);
        path = checkpoint_dir + "/optims/epoch_" + vm["train1_load_epoch"].as<std::string>() + "_vqvae2.pth";  torch::load(optimizer, path, device);
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        if (epoch % vm["train1_save_epoch"].as<size_t>() == 0){
            path = checkpoint_dir + "/models/epoch_" + std::to_string(epoch) + "_vqvae2.pth";  torch::save(model, path);
            path = checkpoint_dir + "/optims/epoch_" + std::to_string(epoch) + "_vqvae2.pth";  torch::save(optimizer, path);
        }
        path = checkpoint_dir + "/models/epoch_latest_vqvae2.pth";  torch::save(model, path);
        path = checkpoint_dir + "/optims/epoch_latest_vqvae2.pth";  torch::save(optimizer, path);
