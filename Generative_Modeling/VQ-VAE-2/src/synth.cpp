...
void synth(po::variables_map &vm, torch::Device &device, VQVAE2 &model, GatedPixelSnail &pixelsnail){
    ...
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" + vm["synth_pixelsnail_load_epoch"].as<std::string>() + "_pixelsnail.pth"; torch::load(pixelsnail, path, device);
    ...
    x = model->sampling(z_shape, pixelsnail, device);
    y = model->sampling(z_shape, pixelsnail, device);
