...
// Function Prototype
void valid2(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device, Loss_PixelSnail &criterion, VQVAE2 &vqvae2, GatedPixelSnail &model, const size_t epoch, visualizer::graph &writer);
...
void train2(po::variables_map &vm, torch::Device &device, VQVAE2 &vqvae2, GatedPixelSnail &model, std::vector<transforms_Compose> &transform){
    ...
    if (vm["train2_load_epoch"].as<std::string>() == ""){
        ...
        path = checkpoint_dir + "/models/epoch_" + vm["train2_load_epoch"].as<std::string>() + "_pixelsnail.pth";  torch::load(model, path, device);
        path = checkpoint_dir + "/optims/epoch_" + vm["train2_load_epoch"].as<std::string>() + "_pixelsnail.pth";  torch::load(optimizer, path, device);
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        if (epoch % vm["train2_save_epoch"].as<size_t>() == 0){
            path = checkpoint_dir + "/models/epoch_" + std::to_string(epoch) + "_pixelsnail.pth";  torch::save(model, path);
            path = checkpoint_dir + "/optims/epoch_" + std::to_string(epoch) + "_pixelsnail.pth";  torch::save(optimizer, path);
        }
        path = checkpoint_dir + "/models/epoch_latest_pixelsnail.pth";  torch::save(model, path);
        path = checkpoint_dir + "/optims/epoch_latest_pixelsnail.pth";  torch::save(optimizer, path);
