...
void sample(po::variables_map &vm, torch::Device &device, VQVAE2 &model, GatedPixelSnail &pixelsnail){
    ...
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" + vm["sample_pixelsnail_load_epoch"].as<std::string>() + "_pixelsnail.pth";  torch::load(pixelsnail, path, device);
    ...
    for (size_t i = 0; i < total; i++){
        ...
        output = model->sampling(z_shape, pixelsnail, device);
