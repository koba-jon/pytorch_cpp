...
void test(po::variables_map &vm, torch::Device &device, LDM &model, std::vector<transforms_Compose> &transform);
void synth(po::variables_map &vm, torch::Device &device, LDM &model);
void sample(po::variables_map &vm, torch::Device &device, LDM &model);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("nt", po::value<size_t>()->default_value(256), "time embedding dimensions")
...
int main(int argc, const char *argv[]){
    ...
    // (5) Define Network
    LDM ldm(vm, device); ldm->to(device);
    LDM ldm_aux(vm, device); ldm_aux->to(device);
    ...
    Set_Model_Params(vm, ldm, "LDM");
    Set_Model_Params(vm, ldm_aux, "LDM_Auxiliary");

    // (8.1) Training Phase
    if (vm["train"].as<bool>()){
        Set_Options(vm, argc, argv, args, "train");
        train(vm, device, ldm, ldm_aux, transform);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, ldm, transform);
    }
