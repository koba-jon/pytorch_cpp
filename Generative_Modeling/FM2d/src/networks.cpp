...
// ---------------------------------------------
// struct{FMImpl}(nn::Module) -> constructor
// ---------------------------------------------
FMImpl::FMImpl(po::variables_map &vm, torch::Device device){
...
// -----------------------------------------------------
// struct{FMImpl}(nn::Module) -> function{add_noise}
// -----------------------------------------------------
std::tuple<torch::Tensor, torch::Tensor> FMImpl::add_noise(torch::Tensor x, torch::Tensor t){
...
// ---------------------------------------------------
// struct{FMImpl}(nn::Module) -> function{ode_step}
// ---------------------------------------------------
torch::Tensor FMImpl::ode_step(torch::Tensor x_t, torch::Tensor t, torch::Tensor dt){
...
// ---------------------------------------------------
// struct{FMImpl}(nn::Module) -> function{forward}
// ---------------------------------------------------
torch::Tensor FMImpl::forward(torch::Tensor x_t, torch::Tensor t){
...
// -----------------------------------------------------
// struct{FMImpl}(nn::Module) -> function{forward_z}
// -----------------------------------------------------
torch::Tensor FMImpl::forward_z(torch::Tensor z){
    torch::Tensor x, t, t_next;
    x = z;
    for (size_t i = 0; i < this->timesteps; i++){
        t = torch::full({z.size(0)}, /*value=*/float(i) / this->timesteps).to(z.device());
        t_next = torch::full({z.size(0)}, /*value=*/float(i + 1) / this->timesteps).to(z.device());
