...
// Sampling Function
// -------------------
void sample(po::variables_map &vm, torch::Device &device, PixelSNAIL &model){
    ...
    // (1) Get Model
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" + vm["sample_load_epoch"].as<std::string>() + ".pth";  torch::load(model, path, device);
    ...
    for (size_t i = 0; i < total; i++){
        ...
        output = model->sampling({1, 1, (long int)vm["size"].as<size_t>(), (long int)vm["size"].as<size_t>()}, device);
