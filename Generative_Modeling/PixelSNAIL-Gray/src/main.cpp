...
// For Original Header
#include "networks.hpp"                // PixelSNAIL
...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, PixelSNAIL &model, std::vector<transforms_Compose> &transform);
void test(po::variables_map &vm, torch::Device &device, PixelSNAIL &model, std::vector<transforms_Compose> &transform);
void synth(po::variables_map &vm, torch::Device &device, PixelSNAIL &pixelsnail);
void sample(po::variables_map &vm, torch::Device &device, PixelSNAIL &pixelsnail);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("L", po::value<size_t>()->default_value(256), "ther number of gradation level")
        ...
        ("train_load_epoch", po::value<std::string>()->default_value(""), "epoch of model to resume learning for training")
        ("save_epoch", po::value<size_t>()->default_value(20), "frequency of epoch to save model and optimizer for training")
        ...
        ("valid_freq", po::value<size_t>()->default_value(1), "validation frequency to training epoch for validation")

        // (4) Define for Test
        ("test", po::value<bool>()->default_value(false), "test mode on/off")
        ("test_dir", po::value<std::string>()->default_value("test"), "test image directory : ./datasets/<dataset>/<test_in_dir>/<image files>")
        ...
        ("dim", po::value<size_t>()->default_value(256), "dimensions of hidden layers of PixelSNAIL")
        ("pix_block", po::value<size_t>()->default_value(4), "the number of pixel blocks")
        ("res_block", po::value<size_t>()->default_value(4), "the number of blocks in residual layer")
        ("res_nc", po::value<size_t>()->default_value(256), "the number of channel in residual layer")
        ("droprate", po::value<float>()->default_value(0.1), "the rate of dropout")
        ("out_res_block", po::value<size_t>()->default_value(0), "the number of out residual block")
...
int main(int argc, const char *argv[]){
    ...
    Set_Model_Params(vm, pixelsnail, "PixelSNAIL");

    // (8.2) Training Phase
    if (vm["train"].as<bool>()){
        Set_Options(vm, argc, argv, args, "train");
        train(vm, device, pixelsnail, transform);
    }

    // (8.3) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, pixelsnail, transform);
    }

    // (8.4) Sampling Phase
    if (vm["sample"].as<bool>()){
        Set_Options(vm, argc, argv, args, "sample");
        sample(vm, device, pixelsnail);
    }
