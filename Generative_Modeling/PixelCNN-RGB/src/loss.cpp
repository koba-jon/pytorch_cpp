...
torch::Tensor Loss::operator()(torch::Tensor input, torch::Tensor target){
    input = input.view({input.size(0), 3, this->L, input.size(2), input.size(3)}).permute({0, 3, 4, 1, 2}).contiguous().view({-1, this->L});
    target = (target * (this->L - 1) + 0.5).to(torch::kLong).permute({0, 2, 3, 1}).contiguous().view(-1);
