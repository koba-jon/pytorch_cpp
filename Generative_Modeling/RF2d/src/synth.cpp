...
void synth(po::variables_map &vm, torch::Device &device, RF &model){
    ...
    // (1) Get Model
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" + vm["synth_load_epoch"].as<std::string>() + "_distill.pth";
    ...
    outputs = model->forward_z(z1, 1);
    for (size_t i = 1; i < count; i++){
        z = z1 * (count - i - 1) / (count - 1) + z2 * i / (count - 1);
        output = model->forward_z(z, 1);
