...
// ---------------------------------------------
// struct{RFImpl}(nn::Module) -> constructor
// ---------------------------------------------
RFImpl::RFImpl(po::variables_map &vm, torch::Device device){
...
// -----------------------------------------------------
// struct{RFImpl}(nn::Module) -> function{add_noise}
// -----------------------------------------------------
std::tuple<torch::Tensor, torch::Tensor> RFImpl::add_noise(torch::Tensor x, torch::Tensor t){
    torch::Tensor z, x_t, v;
    z = torch::randn_like(x);
    x_t = (1.0 - t.view({-1, 1, 1, 1})) * z + t.view({-1, 1, 1, 1}) * x;
    v = x - z;
    return {x_t, v};
}
...
// ---------------------------------------------------
// struct{RFImpl}(nn::Module) -> function{ode_step}
// ---------------------------------------------------
torch::Tensor RFImpl::ode_step(torch::Tensor x_t, torch::Tensor t, torch::Tensor dt){
...
// ---------------------------------------------------
// struct{RFImpl}(nn::Module) -> function{forward}
// ---------------------------------------------------
torch::Tensor RFImpl::forward(torch::Tensor x_t, torch::Tensor t){
...
// -----------------------------------------------------
// struct{RFImpl}(nn::Module) -> function{forward_z}
// -----------------------------------------------------
torch::Tensor RFImpl::forward_z(torch::Tensor z, size_t steps){
    torch::Tensor x, t, t_next;
    x = z;
    if (steps == 1){
        t = torch::zeros({z.size(0)}).to(z.device());
        x = this->ode_step(x, t, torch::ones_like(t));
    }
    else{
        for (size_t i = 0; i < steps; i++){
            t = torch::full({z.size(0)}, /*value=*/float(i) / steps).to(z.device());
            t_next = torch::full({z.size(0)}, /*value=*/float(i + 1) / steps).to(z.device());
            x = this->ode_step(x, t, t_next - t);  // {C,256,256} ===> {C,256,256}
        }
    }
    return x;
}

torch::Tensor RFImpl::forward_z(torch::Tensor z){
    return this->forward_z(z, this->timesteps);
}
