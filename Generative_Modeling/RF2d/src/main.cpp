...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, RF &model, RF &model_aux, std::vector<transforms_Compose> &transform);
void test(po::variables_map &vm, torch::Device &device, RF &model, std::vector<transforms_Compose> &transform);
...
void synth(po::variables_map &vm, torch::Device &device, RF &model);
void sample(po::variables_map &vm, torch::Device &device, RF &model);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("timesteps", po::value<size_t>()->default_value(100), "total time steps for inference")
        ...
        ("reflow", po::value<bool>()->default_value(false), "sampling mode on/off")
        ("reflow_load_epoch", po::value<std::string>()->default_value("latest"), "training epoch used for sampling")
        ("reflow_in_dir", po::value<std::string>()->default_value("reflowI"), "reflow input image directory : ./datasets/<dataset>/<reflow_in_dir>/<image files>")
        ("reflow_out_dir", po::value<std::string>()->default_value("reflowO"), "reflow output image directory : ./datasets/<dataset>/<reflow_out_dir>/<image files>")
        ("reflow_total", po::value<size_t>()->default_value(10000), "total number of data obtained by random sampling")

        // (6) Define for Distillation
        ("distill", po::value<bool>()->default_value(false), "distillation mode on/off")
        ("distill_epochs", po::value<size_t>()->default_value(200), "distillation total epoch")
        ("distill_batch_size", po::value<size_t>()->default_value(32), "distillation batch size")
        ("distill_load_epoch", po::value<std::string>()->default_value(""), "epoch of model to resume learning")
        ("distill_save_epoch", po::value<size_t>()->default_value(20), "frequency of epoch to save model and optimizer")

        // (7) Define for Synthesis
        ...
        // (9) Define for Network Parameter
...
int main(int argc, const char *argv[]){
    ...
    // (5) Define Network
    RF rf(vm, device); rf->to(device);
    RF rf_aux(vm, device); rf_aux->to(device);
    ...
    Set_Model_Params(vm, rf, "RF");

    // (8.1) Training Phase
    if (vm["train"].as<bool>()){
        Set_Options(vm, argc, argv, args, "train");
        train(vm, device, rf, rf_aux, transform);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, rf, transform);
    }

    // (8.3) Reflow Phase
    if (vm["reflow"].as<bool>()){
        Set_Options(vm, argc, argv, args, "reflow");
        reflow(vm, device, rf);
    }

    // (8.4) Distillation Phase
    if (vm["distill"].as<bool>()){
        Set_Options(vm, argc, argv, args, "distill");
        distill(vm, device, rf);
    }
