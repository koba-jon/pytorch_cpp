...
// Function Prototype
void valid(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device, Loss &criterion, RF &model, const size_t epoch, visualizer::graph &writer);
void ema(RF &source, RF &target, const float decay);
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        while (dataloader(mini_batch)){
            ...
            if (iter % save_sample_iter == 1){
                ...
                recon = x_t + (1.0 - t.view({-1, 1, 1, 1})) * output;
