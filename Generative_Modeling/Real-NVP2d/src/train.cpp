...
void train(po::variables_map &vm, torch::Device &device, NormalizingFlow &model, std::vector<transforms_Compose> &transform){
    ...
    torch::Tensor zK, sum_logdet, loss, image, output, pair;
    ...
    train_loss = visualizer::graph(path, /*gname_=*/"train_loss", /*label_=*/{"Loss"});
    if (vm["valid"].as<bool>()){
        valid_loss = visualizer::graph(path, /*gname_=*/"valid_loss", /*label_=*/{"Loss"});
    }
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        while (dataloader(mini_batch)){
            ...
            // -----------------------------------
            // c1. NormalizingFlow Training Phase
            // -----------------------------------
            std::tie(zK, sum_logdet) = model->forward(image.view({image.size(0), -1}));
            loss = criterion(zK, sum_logdet);
            optimizer.zero_grad();
            loss.backward();
            for (auto &p : model->parameters()) if (p.grad().defined()) p.grad().data().clamp_(-5.0, 5.0);
            optimizer.step();
            ...
            if (iter % save_sample_iter == 1){
                ...
                output = model->inverse(zK).view(image.sizes());
                pair = torch::cat({image, output}, 0);
