...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, DiT &model, DiT &model_aux, std::vector<transforms_Compose> &transform);
void test(po::variables_map &vm, torch::Device &device, DiT &model, std::vector<transforms_Compose> &transform);
void synth(po::variables_map &vm, torch::Device &device, DiT &model);
void sample(po::variables_map &vm, torch::Device &device, DiT &model);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("nz", po::value<size_t>()->default_value(16), "latent channels for the autoencoder")
        ("ae_downsample", po::value<size_t>()->default_value(4), "down-sampling times inside the autoencoder")
        ("patch", po::value<size_t>()->default_value(4), "patch size for tokenization")
        ("dim", po::value<size_t>()->default_value(768), "embedding dimension for transformer")
        ("depth", po::value<size_t>()->default_value(12), "number of transformer blocks")
        ("heads", po::value<size_t>()->default_value(12), "number of attention heads")
        ("dim_head", po::value<size_t>()->default_value(64), "dimension of each attention head")
        ("mlp_dim", po::value<size_t>()->default_value(2048), "hidden dimension in MLP of transformer block")
        ...
        ("Lambda", po::value<float>()->default_value(0.1), "weight of reconstruction loss for the autoencoder")
...
int main(int argc, const char *argv[]){
    ...
    // (5) Define Network
    DiT dit(vm, device); dit->to(device);
    DiT dit_aux(vm, device); dit_aux->to(device);
    ...
    Set_Model_Params(vm, dit, "DiT");
    Set_Model_Params(vm, dit_aux, "DiT_Auxiliary");

    // (8.1) Training Phase
    if (vm["train"].as<bool>()){
        Set_Options(vm, argc, argv, args, "train");
        train(vm, device, dit, dit_aux, transform);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, dit, transform);
    }
