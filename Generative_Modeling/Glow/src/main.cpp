...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, NormalizingFlow &model, std::vector<transforms_Compose> &transform);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("size", po::value<size_t>()->default_value(32), "image width and height")
        ("nc", po::value<size_t>()->default_value(3), "input image channel : RGB=3, grayscale=1")
        ("n_flow", po::value<size_t>()->default_value(32), "the number of flow in a block")
        ("n_block", po::value<size_t>()->default_value(4), "the number of block")
        ("n_bits", po::value<size_t>()->default_value(5), "the number of bits")
        ...
        ("h_dim", po::value<size_t>()->default_value(512), "the number of hidden dimensions in coupling layer")
...
int main(int argc, const char *argv[]){
    ...
    Set_Model_Params(vm, NF, "Normalizing Flow");

    // (8.1) Training Phase
    if (vm["train"].as<bool>()){
        Set_Options(vm, argc, argv, args, "train");
        train(vm, device, NF, transform);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, NF, transform);
    }
