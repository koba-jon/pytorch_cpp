...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, PixelCNN &model, std::vector<transforms_Compose> &transform);
void test(po::variables_map &vm, torch::Device &device, PixelCNN &model, std::vector<transforms_Compose> &transform);
void synth(po::variables_map &vm, torch::Device &device, PixelCNN &pixelcnn);
void sample(po::variables_map &vm, torch::Device &device, PixelCNN &pixelcnn);
...
int main(int argc, const char *argv[]){
    ...
    if (vm["train"].as<bool>()){
        ...
        train(vm, device, pixelcnn, transform);
    }

    // (8.3) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, pixelcnn, transform);
    }

    // (8.4) Sampling Phase
    if (vm["sample"].as<bool>()){
        Set_Options(vm, argc, argv, args, "sample");
        sample(vm, device, pixelcnn);
    }
