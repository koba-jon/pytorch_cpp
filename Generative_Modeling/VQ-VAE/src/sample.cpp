...
void sample(po::variables_map &vm, torch::Device &device, VQVAE &model, PixelCNN &pixelcnn){
    ...
    // (1) Get Model
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" + vm["sample_vqvae_load_epoch"].as<std::string>() + "_vqvae.pth";  torch::load(model, path, device);
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" + vm["sample_pixelcnn_load_epoch"].as<std::string>() + "_pixelcnn.pth";  torch::load(pixelcnn, path, device);
    ...
    for (size_t i = 0; i < total; i++){
        ...
        output = model->sampling(z_shape, pixelcnn, device);
