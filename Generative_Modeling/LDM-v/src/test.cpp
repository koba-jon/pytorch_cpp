...
void test(po::variables_map &vm, torch::Device &device, LDM &model, std::vector<transforms_Compose> &transform){
    ...
    torch::Tensor imageI, imageO, t, pred_v, target_v, rec, latent_noisy, loss, GT_loss;
    ...
    while (dataloader(data)){
        ...
        std::tie(pred_v, target_v, rec, latent_noisy) = model->forward(imageI, t);
        ...
        std::cout << '<' << std::get<2>(data).at(0) << "> Diff_L2:" << loss.item<float>() << " GT_L2:" << GT_loss.item<float>() << std::endl;
        ofs << '<' << std::get<2>(data).at(0) << "> Diff_L2:" << loss.item<float>() << " GT_L2:" << GT_loss.item<float>() << std::endl;

        fname = result_dir + '/' + std::get<3>(data).at(0);
        visualizer::save_image(rec.detach(), fname, /*range=*/output_range, /*cols=*/1, /*padding=*/0);
    ...
    // (7) Average Output
    std::cout << "<All> Diff_L2:" << ave_loss << " GT_L2:" << ave_GT_loss << " (time:" << ave_time << ')' << std::endl;
    ofs << "<All> Diff_L2:" << ave_loss << " GT_L2:" << ave_GT_loss << " (time:" << ave_time << ')' << std::endl;
