...
// Function Prototype
void valid(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device, Loss &criterion, LDM &model, const size_t epoch, visualizer::graph &writer);
void ema(LDM &source, LDM &target, const float decay);


// -------------------
// Training Function
// -------------------
void train(po::variables_map &vm, torch::Device &device, LDM &model, LDM &model_aux, std::vector<transforms_Compose> &transform){
    ...
    torch::Tensor image, t, pred_v, target_v, rec, latent_noisy, loss, loss_diff, loss_rec, noisy_image, clean_image, pair;
    ...
    train_loss = visualizer::graph(path, /*gname_=*/"train_loss", /*label_=*/{"Total", "Diffusion", "Reconstruct"});
    if (vm["valid"].as<bool>()){
        valid_loss = visualizer::graph(path, /*gname_=*/"valid_loss", /*label_=*/{"Total", "Diffusion", "Reconstruct"});
    }
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        show_progress = new progress::display(/*count_max_=*/total_iter, /*epoch=*/{epoch, total_epoch}, /*loss_=*/{"loss", "diff", "rec"});
        ...
        while (dataloader(mini_batch)){
            ...
            // -------------------------
            // c1. LDM Training Phase
            // -------------------------
            t = torch::randint(1, vm["timesteps"].as<size_t>() + 1, {mini_batch_size}).to(device);
            std::tie(pred_v, target_v, rec, latent_noisy) = model_aux->forward(image, t);
            loss_diff = criterion(pred_v, target_v);
            ...
            // -----------------------------------
            show_progress->increment(/*loss_value=*/{loss.item<float>(), loss_diff.item<float>(), loss_rec.item<float>()});
            ofs << "iters:" << show_progress->get_iters() << '/' << total_iter << ' ' << std::flush;
            ofs << "loss:" << loss.item<float>() << "(ave:" <<  show_progress->get_ave(0) << ") " << std::flush;
            ofs << "diff:" << loss_diff.item<float>() << "(ave:" <<  show_progress->get_ave(1) << ") " << std::flush;
            ofs << "rec:" << loss_rec.item<float>() << "(ave:" <<  show_progress->get_ave(2) << ')' << std::endl;
            ...
            if (iter % save_sample_iter == 1){
                ...
                noisy_image = model->ae->decode(latent_noisy);
                clean_image = model->ae->decode(model->denoise_t(latent_noisy, t));
                pair = torch::cat({image, rec, noisy_image, clean_image}, /*dim=*/0);
        ...
        // -----------------------------------
        train_loss.plot(/*base=*/epoch, /*value=*/{show_progress->get_ave(0), show_progress->get_ave(1), show_progress->get_ave(2)});
