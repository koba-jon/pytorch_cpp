...
// For Original Header
#include "networks.hpp"                // MaskedAutoEncoder
...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, MaskedAutoEncoder &model, std::vector<transforms_Compose> &transform);
void test(po::variables_map &vm, torch::Device &device, MaskedAutoEncoder &model, std::vector<transforms_Compose> &transform);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("enc_heads", po::value<size_t>()->default_value(16), "the number of head in Transformer for encoder")
        ("dec_heads", po::value<size_t>()->default_value(16), "the number of head in Transformer for decoder")
        ("enc_dim", po::value<size_t>()->default_value(1024), "dimensions of the embedded patch features for encoder")
        ("dec_dim", po::value<size_t>()->default_value(512), "dimensions of the embedded patch features for decoder")
        ("mlp_ratio", po::value<float>()->default_value(4.0), "ratio of dimensions in multi layer perceptron")
...
int main(int argc, const char *argv[]){
    ...
    Set_Model_Params(vm, MAE, "MAE");

    // (8.1) Training Phase
    if (vm["train"].as<bool>()){
        Set_Options(vm, argc, argv, args, "train");
        train(vm, device, MAE, transform);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, MAE, transform);
    }
