...
// Function Prototype
void valid(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device, Loss &criterion, MaskedAutoEncoder &model, const size_t epoch, visualizer::graph &writer);
...
void train(po::variables_map &vm, torch::Device &device, MaskedAutoEncoder &model, std::vector<transforms_Compose> &transform){
    ...
    torch::Tensor loss, image, patch, output, mask, noise, pair;
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        while (dataloader(mini_batch)){
            ...
            // -----------------------------------
            output_set = model->forward(image);
            patch = std::get<0>(output_set);
            output = std::get<1>(output_set);
            mask = std::get<2>(output_set);
            loss = criterion(patch, output, mask);
            optimizer.zero_grad();
            loss.backward();
            optimizer.step();
