...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, SimCLR &model, std::vector<transforms_Compose> &transform);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("base_lr", po::value<float>()->default_value(0.3), "base learning rate")
        ("momentum", po::value<float>()->default_value(0.9), "momentum of SGD")
        ("weight_decay", po::value<float>()->default_value(1e-6), "weight decay of SGD")
        ("nf", po::value<size_t>()->default_value(64), "the number of filters in convolution layer closest to image")
        ("nz", po::value<size_t>()->default_value(128), "the number of final features")
        ("n_layers", po::value<size_t>()->default_value(50), "the number of layer in model")
        ("jitter_prob", po::value<double>()->default_value(0.8), "probability of color jitter")
        ("drop_prob", po::value<double>()->default_value(0.2), "probability of color drop")
        ("jitter", po::value<float>()->default_value(0.8), "strength of color jitter for brightness, contrast and saturation")
        ("jitter_hue", po::value<float>()->default_value(0.2), "strength of color jitter for hue")
...
int main(int argc, const char *argv[]){
    ...
    Set_Model_Params(vm, model, "SimCLR");
    ...
    if (vm["test"].as<bool>()){
        ...
        test(vm, device, model, transform);
    }
