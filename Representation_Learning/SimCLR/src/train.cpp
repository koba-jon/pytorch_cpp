...
void train(po::variables_map &vm, torch::Device &device, SimCLR &model, std::vector<transforms_Compose> &transform){
    ...
    torch::Tensor loss, image, aug1, aug2, z1, z2, pair;
    ...
    dataloader = DataLoader::ImageFolderWithPaths(dataset, vm["batch_size"].as<size_t>(), /*shuffle_=*/train_shuffle, /*num_workers_=*/train_workers, /*pin_memory_=*/false, /*drop_last_=*/true);
    ...
    auto optimizer = torch::optim::SGD(model->parameters(), torch::optim::SGDOptions(vm["base_lr"].as<float>() * vm["batch_size"].as<size_t>() / 256.0).momentum(vm["momentum"].as<float>()).weight_decay(vm["weight_decay"].as<float>()));
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        while (dataloader(mini_batch)){
            ...
            // -----------------------------------
            // c1. Masked Auto Encoder Training Phase
            // -----------------------------------
            ...
            if (iter % save_sample_iter == 1){
                ...
                pair = torch::cat({image, aug1, aug2}, /*dim=*/0);
