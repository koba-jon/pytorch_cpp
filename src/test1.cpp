...
void test1(po::variables_map &vm, torch::Device &device, VQVAE2 &model, std::vector<transforms_Compose> &transform){
    ...
    float ave_rec_loss, ave_latent_loss, ave_GT_loss;
    ...
    torch::Tensor rec, latent, GT_loss;
    datasets::ImageFolderPairWithPaths dataset;
    DataLoader::ImageFolderPairWithPaths dataloader;

    // (1) Get Test Dataset
    input_dir = "datasets/" + vm["dataset"].as<std::string>() + '/' + vm["test1_in_dir"].as<std::string>();
    output_dir = "datasets/" + vm["dataset"].as<std::string>() + '/' + vm["test1_out_dir"].as<std::string>();
    ...
    // (2) Get Model
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" + vm["test1_load_epoch"].as<std::string>() + "_vqvae2.pth";
    ...
    model->eval();
    result_dir = vm["test1_result_dir"].as<std::string>();  fs::create_directories(result_dir);
    ...
    while (dataloader(data)){
        ...
        ave_latent_loss += latent.item<float>();
        ave_GT_loss += GT_loss.item<float>();
        ave_time += seconds;

        std::cout << '<' << std::get<2>(data).at(0) << "> " << vm["loss"].as<std::string>() << ':' << rec.item<float>() << " latent:" << latent.item<float>() << " GT_" << vm["loss"].as<std::string>() << ':' << GT_loss.item<float>() << std::endl;
        ofs << '<' << std::get<2>(data).at(0) << "> " << vm["loss"].as<std::string>() << ':' << rec.item<float>() << " latent:" << latent.item<float>() << " GT_" << vm["loss"].as<std::string>() << ':' << GT_loss.item<float>() << std::endl;
    ...
    ave_latent_loss = ave_latent_loss / (float)dataset.size();
    ave_GT_loss = ave_GT_loss / (float)dataset.size();
    ave_time = ave_time / (double)dataset.size();

    // (7) Average Output
    std::cout << "<All> " << vm["loss"].as<std::string>() << ':' << ave_rec_loss << " latent:" << ave_latent_loss << " GT_" << vm["loss"].as<std::string>() << ':' << ave_GT_loss << " (time:" << ave_time << ')' << std::endl;
    ofs << "<All> " << vm["loss"].as<std::string>() << ':' << ave_rec_loss << " latent:" << ave_latent_loss << " GT_" << vm["loss"].as<std::string>() << ':' << ave_GT_loss << " (time:" << ave_time << ')' << std::endl;
