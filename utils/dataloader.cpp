...
// --------------------------------------------------------------------
// namespace{DataLoader} -> class{ImageTensorFolderPairWithPaths} -> constructor
// --------------------------------------------------------------------
DataLoader::ImageTensorFolderPairWithPaths::ImageTensorFolderPairWithPaths(datasets::ImageTensorFolderPairWithPaths &dataset_, const size_t batch_size_, const bool shuffle_, const size_t num_workers_, const bool pin_memory_, const bool drop_last_){
...
// --------------------------------------------------------------------
// namespace{DataLoader} -> class{ImageTensorFolderPairWithPaths} -> operator
// --------------------------------------------------------------------
bool DataLoader::ImageTensorFolderPairWithPaths::operator()(std::tuple<torch::Tensor, torch::Tensor, std::vector<std::string>, std::vector<std::string>> &data){
...
// ----------------------------------------------------------------------------
// namespace{DataLoader} -> class{ImageTensorFolderPairWithPaths} -> function{reset}
// ----------------------------------------------------------------------------
void DataLoader::ImageTensorFolderPairWithPaths::reset(){
    this->count = 0;
    return;
}
...
// namespace{DataLoader} -> class{ImageFolderPairWithPaths} -> function{get_count_max}
// ------------------------------------------------------------------------------------
size_t DataLoader::ImageTensorFolderPairWithPaths::get_count_max(){
    return this->count_max;
}
...
// -------------------------------------------------------------------------------
// namespace{DataLoader} -> class{ImageFolderCameraPoseWithPaths} -> constructor
// -------------------------------------------------------------------------------
DataLoader::ImageFolderCameraPoseWithPaths::ImageFolderCameraPoseWithPaths(datasets::ImageFolderCameraPoseWithPaths &dataset_, const size_t batch_size_, const bool shuffle_, const size_t num_workers_, const bool pin_memory_, const bool drop_last_){
...
// ----------------------------------------------------------------------------
// namespace{DataLoader} -> class{ImageFolderCameraPoseWithPaths} -> operator
// ----------------------------------------------------------------------------
bool DataLoader::ImageFolderCameraPoseWithPaths::operator()(std::tuple<torch::Tensor, torch::Tensor, std::vector<std::string>, std::vector<std::string>> &data){
    ...
    for (i = 1; i < mini_batch_size; i++){
        ...
        data1 = torch::cat({data1, tensor1}, /*dim=*/0);  // {i,C,H,W} + {1,C,H,W} ===> {i+1,C,H,W}
        tensor2 = std::get<1>(group);
        tensor2 = torch::unsqueeze(tensor2, /*dim=*/0);  // {4,4} ===> {1,4,4}
        data2 = torch::cat({data2, tensor2}, /*dim=*/0);  // {i,4,4} + {1,4,4} ===> {i+1,4,4}
        data3.push_back(std::get<2>(group));
        data4.push_back(std::get<3>(group));
    }
    data1 = data1.contiguous().detach().clone();
    ...
    // Post Processing
    this->count++;
    data = {data1, data2, data3, data4};  // {N,C,H,W} (images), {N,4,4} (pose), {N} (fnames1), {N} (fnames2)
    delete[] data_before;
...
// -----------------------------------------------------------------------------------
// namespace{DataLoader} -> class{ImageFolderCameraPoseWithPaths} -> function{reset}
// -----------------------------------------------------------------------------------
void DataLoader::ImageFolderCameraPoseWithPaths::reset(){
    this->count = 0;
    return;
}
...
// -------------------------------------------------------------------------------------------
// namespace{DataLoader} -> class{ImageFolderCameraPoseWithPaths} -> function{get_count_max}
// -------------------------------------------------------------------------------------------
size_t DataLoader::ImageFolderCameraPoseWithPaths::get_count_max(){
    return this->count_max;
}
