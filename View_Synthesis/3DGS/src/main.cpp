...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, GS3D &model, std::vector<transforms_Compose> &transform);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("lr_init", po::value<float>()->default_value(5e-3), "initial learning rate")
        ("lr_decay", po::value<float>()->default_value(5e-4), "decay learning rate")
