...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> Loss::build_target(std::vector<std::tuple<torch::Tensor, torch::Tensor>> &target, const long int ng){
    ...
    target_class = torch::full({mini_batch_size, ng, ng, this->class_num}, /*value=*/0.0, torch::TensorOptions().dtype(torch::kFloat));  // target_class{N,G,G,CN}
    target_coord = torch::full({mini_batch_size, ng, ng, 4}, /*value=*/0.0, torch::TensorOptions().dtype(torch::kFloat));  // target_coord{N,G,G,4}
    target_mask = torch::full({mini_batch_size, ng, ng}, /*value=*/false, torch::TensorOptions().dtype(torch::kBool));  // target_mask{N,G,G}

    for (long int n = 0; n < mini_batch_size; n++){
        data = target.at(n);
        ids = std::get<0>(data);  // {BB_n}
        coords = std::get<1>(data);  // {BB_n,4}
        BB_n = ids.size(0);
        for (long int b = 0; b < BB_n; b++){
            ...
            i = (long int)(cx / cell_size);  // i[0,G)
            j = (long int)(cy / cell_size);  // j[0,G)
            if (target_mask.index({n, j, i}).item<bool>() == false){
                target_class.index_put_({n, j, i, id}, 1.0);  // class likelihood = 1.0
                target_coord.index_put_({n, j, i, 0}, cx);  // center of x[0.0,1.0)
                target_coord.index_put_({n, j, i, 1}, cy);  // center of y[0.0,1.0)
                target_coord.index_put_({n, j, i, 2}, w);  // width[0.0,1.0)
                target_coord.index_put_({n, j, i, 3}, h);  // height[0.0,1.0)
                target_mask.index_put_({n, j, i}, true);  // mask = true
            }
        }
    }
