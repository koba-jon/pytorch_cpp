...
Loss::Loss(const std::vector<std::tuple<float, float>> anchors_, const long int class_num_, const float thresh_){
    ...
    this->anchors_wh = torch::from_blob(anchors.data(), {1, 1, 1, this->na, 2}, torch::kFloat).clone();  // anchors_wh{1,1,1,A,2}
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> Loss::operator()(torch::Tensor &input, std::vector<std::tuple<torch::Tensor, torch::Tensor>> &target){
    ...
    input_wh_unoffset = torch::exp(input_wh) * this->anchors_wh.to(device) / (float)ng;  // input_wh{N,G,G,A,2} ===> input_wh_unoffset{N,G,G,A,2}
    ...
    target_wh = torch::log(target_wh_unoffset * (float)ng / this->anchors_wh.to(device));  // target_wh_unoffset{N,G,G,A,2} ===> target_wh{N,G,G,A,2}
    /*************************************************************************/
    ...
    no_response_coord_mask = no_response_mask.unsqueeze(/*dim=*/-1).expand_as(input_xy);  // no_response_mask{N,G,G,A} ===> {N,G,G,A,1} ===> no_response_coord_mask{N,G,G,A,2}
    ...
    /*************************************************************************/
    if (no_response_flag){
        input_no_response_xy = input_xy.masked_select(/*mask=*/no_response_coord_mask);  // input_xy{N,G,G,A,2} ===> input_no_response_xy{no response*2}
        target_no_response_xy = target_xy_noobj.masked_select(/*mask=*/no_response_coord_mask);  // target_xy_noobj{N,G,G,A,2} ===> target_no_response_xy{no response*2}
        loss_no_response_coord_xy = criterion(input_no_response_xy, target_no_response_xy) * 0.5 * no_response_coord_weight / (float)mini_batch_size;
    }
    else{
    ...
    /*************************************************************************/
    if (no_response_flag){
        input_no_response_wh = input_wh.masked_select(/*mask=*/no_response_coord_mask);  // input_wh{N,G,G,A,2} ===> input_no_response_wh{no response*2}
        target_no_response_wh = target_wh_noobj.masked_select(/*mask=*/no_response_coord_mask);  // target_wh_noobj{N,G,G,A,2} ===> target_no_response_wh{no response*2}
        loss_no_response_coord_wh = criterion(input_no_response_wh, target_no_response_wh) * 0.5 * no_response_coord_weight / (float)mini_batch_size;
    }
    else{
    ...
    if (response_flag){
        ...
        IoU = IoU_grid.view({this->na, mini_batch_size, ng, ng}).permute({1, 2, 3, 0}).contiguous();  // IoU_grid{A,N*G*G} ===> {A,N,G,G} ===> IoU{N,G,G,A}
