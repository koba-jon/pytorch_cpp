...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("nz_c", po::value<size_t>()->default_value(512), "dimensions of compression features in latent space")
        ("nz_r", po::value<size_t>()->default_value(2), "dimensions of reconstruction features in latent space")
        ("nk", po::value<size_t>()->default_value(10), "number of mixture component")
        ...
        ("com_detach", po::value<bool>()->default_value(false), "calculation graph detachment on/off in compression features")
        ("rec_detach", po::value<bool>()->default_value(true), "calculation graph detachment on/off in reconstruction features")
        ...
        ("nr", po::value<size_t>()->default_value(1024), "number of node for residual block in estimation network")
        ...
        ("n_blocks", po::value<size_t>()->default_value(3), "number of residual blocks in estimation network")
...
int main(int argc, const char *argv[]){
    ...
    // (5) Define Network
    Encoder enc(vm); enc->to(device);
    Decoder dec(vm); dec->to(device);
    EstimationNetwork est(vm); est->to(device);
