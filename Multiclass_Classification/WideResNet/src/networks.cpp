...
// ----------------------------------------------------------------------
// struct{MC_WideResNetImpl}(nn::Module) -> constructor
// ----------------------------------------------------------------------
MC_WideResNetImpl::MC_WideResNetImpl(po::variables_map &vm){
    ...
    if (n_layers == 50){
        ...
        std::cerr << "Error : Please choose 50 or 101." << std::endl;
        std::exit(1);
    }
    size_t feature = vm["nf"].as<size_t>() * 2;
    ...
    // Classification
    this->classifier = nn::Sequential(nn::Linear(/*in_channels=*/feature*8*block.expansion, /*out_channels=*/vm["class_num"].as<size_t>()));  // {8F*E} ===> {CN}
...
// ---------------------------------------------------------
// struct{MC_WideResNetImpl}(nn::Module) -> function{init}
// ---------------------------------------------------------
void MC_WideResNetImpl::init(){
...
// -----------------------------------------------------------
// struct{MC_WideResNetImpl}(nn::Module) -> function{make_layers}
// -----------------------------------------------------------
template <typename T>
nn::Sequential MC_WideResNetImpl::make_layers(T &block, const size_t planes, const size_t num_blocks, const size_t stride){
...
// ---------------------------------------------------------
// struct{MC_WideResNetImpl}(nn::Module) -> function{forward}
// ---------------------------------------------------------
torch::Tensor MC_WideResNetImpl::forward(torch::Tensor x){
