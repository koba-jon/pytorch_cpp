...
#include "networks.hpp"                // MC_EfficientNet
...
// Function Prototype
void valid(po::variables_map &vm, DataLoader::ImageFolderClassesWithPaths &valid_dataloader, torch::Device &device, Loss &criterion, MC_EfficientNet &model, const std::vector<std::string> class_names, const size_t epoch, visualizer::graph &writer, visualizer::graph &writer_accuracy, visualizer::graph &writer_each_accuracy);


// -------------------
// Training Function
// -------------------
void train(po::variables_map &vm, torch::Device &device, MC_EfficientNet &model, std::vector<transforms_Compose> &transform, const std::vector<std::string> class_names){
    ...
    dataloader = DataLoader::ImageFolderClassesWithPaths(dataset, vm["batch_size"].as<size_t>(), /*shuffle_=*/train_shuffle, /*num_workers_=*/train_workers);
    ...
    if (vm["valid"].as<bool>()){
        ...
        valid_dataloader = DataLoader::ImageFolderClassesWithPaths(valid_dataset, vm["valid_batch_size"].as<size_t>(), /*shuffle_=*/valid_shuffle, /*num_workers_=*/valid_workers);
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        while (dataloader(mini_batch)){
            ...
            // -----------------------------------
            // c1. EfficientNet Training Phase
            // -----------------------------------
        ...
        if (vm["valid"].as<bool>() && ((epoch - 1) % vm["valid_freq"].as<size_t>() == 0)){
            valid(vm, valid_dataloader, device, criterion, model, class_names, epoch, valid_loss, valid_accuracy, valid_each_accuracy);
        }
