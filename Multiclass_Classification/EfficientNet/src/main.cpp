...
void test(po::variables_map &vm, torch::Device &device, MC_EfficientNet &model, std::vector<transforms_Compose> &transform, const std::vector<std::string> class_names);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("network", po::value<std::string>()->default_value("B0"), "the type of network : B0, B1, B2, B3, B4, B5, B6, B7, B8, L2")
...
int main(int argc, const char *argv[]){
    ...
    // (4) Define Network
    MC_EfficientNet model(vm);
    model->to(device);
    
    // (5) Set Transforms
    std::vector<transforms_Compose> transform{
        transforms_Resize(cv::Size(model->cfg.image_size, model->cfg.image_size), cv::INTER_LINEAR),        // {IH,IW,C} ===method{OW,OH}===> {OH,OW,C}
    ...
    Set_Model_Params(vm, model, "EfficientNet");
