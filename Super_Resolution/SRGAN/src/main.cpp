...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("train_lr_dir", po::value<std::string>()->default_value("trainLR"), "training low-resolution image directory : ./datasets/<dataset>/<train_lr_dir>/<image files>")
        ("train_hr_dir", po::value<std::string>()->default_value("trainHR"), "training high-resolution image directory : ./datasets/<dataset>/<train_hr_dir>/<image files>")
        ...
        ("valid_lr_dir", po::value<std::string>()->default_value("validLR"), "validation low-resolution image directory : ./datasets/<dataset>/<valid_lr_dir>/<image files>")
        ("valid_hr_dir", po::value<std::string>()->default_value("validHR"), "validation high-resolution image directory : ./datasets/<dataset>/<valid_hr_dir>/<image files>")
        ...
        ("test_lr_dir", po::value<std::string>()->default_value("testLR"), "test low-resolution image directory : ./datasets/<dataset>/<test_lr_dir>/<image files>")
        ("test_hr_dir", po::value<std::string>()->default_value("testHR"), "test high-resolution image directory : ./datasets/<dataset>/<test_hr_dir>/<image files>")
...
int main(int argc, const char *argv[]){
    ...
    // (4) Set Transforms
    size_t hr_size = vm["hr_size"].as<size_t>();
    size_t lr_size = hr_size / vm["upscale"].as<size_t>();
    std::vector<transforms_Compose> transformLR{
        transforms_Resize(cv::Size(lr_size, lr_size), cv::INTER_LINEAR),  // {IH,IW,C} ===method{OW,OH}===> {OH,OW,C}
        transforms_ToTensor(),                                                                            // Mat Image [0,255] or [0,65535] ===> Tensor Image [0,1]
        transforms_Normalize(0.5, 0.5)                                                                    // [0,1] ===> [-1,1]
    };
    if (vm["nc"].as<size_t>() == 1){
        transformLR.insert(transformLR.begin(), transforms_Grayscale(1));
    }
    std::vector<transforms_Compose> transformHR{
        transforms_Resize(cv::Size(hr_size, hr_size), cv::INTER_LINEAR),  // {IH,IW,C} ===method{OW,OH}===> {OH,OW,C}
        transforms_ToTensor(),                                                                            // Mat Image [0,255] or [0,65535] ===> Tensor Image [0,1]
        transforms_Normalize(0.5, 0.5)                                                                    // [0,1] ===> [-1,1]
    };
    if (vm["nc"].as<size_t>() == 1){
        transformHR.insert(transformHR.begin(), transforms_Grayscale(1));
    }
    ...
    if (vm["train"].as<bool>()){
        ...
        train(vm, device, gen, dis, transformLR, transformHR);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, gen, transformLR, transformHR);
    }
