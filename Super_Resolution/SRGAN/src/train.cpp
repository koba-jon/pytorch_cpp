...
void train(po::variables_map &vm, torch::Device &device, SRGAN_Generator &gen, SRGAN_Discriminator &dis, std::vector<transforms_Compose> &transformLR, std::vector<transforms_Compose> &transformHR){
    ...
    std::string lr_dir, hr_dir;
    std::string valid_lr_dir, valid_hr_dir;
    std::stringstream ss;
    std::ifstream infoi;
    ...
    torch::Tensor gen_loss, G_MSE_loss, G_content_loss, G_GAN_loss;
    ...
    lr_dir = "datasets/" + vm["dataset"].as<std::string>() + "/" + vm["train_lr_dir"].as<std::string>();
    hr_dir = "datasets/" + vm["dataset"].as<std::string>() + "/" + vm["train_hr_dir"].as<std::string>();
    dataset = datasets::ImageFolderPairWithPaths(lr_dir, hr_dir, transformLR, transformHR);
    ...
    if (vm["valid"].as<bool>()){
        valid_lr_dir = "datasets/" + vm["dataset"].as<std::string>() + "/" + vm["valid_lr_dir"].as<std::string>();
        valid_hr_dir = "datasets/" + vm["dataset"].as<std::string>() + "/" + vm["valid_hr_dir"].as<std::string>();
        valid_dataset = datasets::ImageFolderPairWithPaths(valid_lr_dir, valid_hr_dir, transformLR, transformHR);
    ...
    // (4) Set Loss Function
    auto criterion_MSE = torch::nn::MSELoss(torch::nn::MSELossOptions().reduction(torch::kMean));
    ...
    train_loss_gen = visualizer::graph(path, /*gname_=*/"train_loss_gen", /*label_=*/{"Total", "MSE", "GAN", "Content"});
    ...
    if (vm["valid"].as<bool>()){
        ...
        valid_loss_gen = visualizer::graph(path, /*gname_=*/"valid_loss_gen", /*label_=*/{"Total", "MSE", "GAN", "Content"});
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        show_progress = new progress::display(/*count_max_=*/total_iter, /*epoch=*/{epoch, total_epoch}, /*loss_=*/{"G_MSE", "G_GAN", "G_Con", "D_Real", "D_Fake"});
        ...
        while (dataloader(mini_batch)){
            ...
            gen_loss = G_MSE_loss + G_GAN_loss + G_content_loss;
            ...
            // -----------------------------------
            show_progress->increment(/*loss_value=*/{G_MSE_loss.item<float>(), G_GAN_loss.item<float>(), G_content_loss.item<float>(), dis_real_loss.item<float>(), dis_fake_loss.item<float>()});
            ofs << "iters:" << show_progress->get_iters() << '/' << total_iter << ' ' << std::flush;
            ofs << "G_MSE:" << G_MSE_loss.item<float>() << "(ave:" <<  show_progress->get_ave(0) << ") " << std::flush;
            ...
            if (iter % save_sample_iter == 1){
                ...
                lr = F::interpolate(lr, F::InterpolateFuncOptions().size(std::vector<long int>{hr.size(2), hr.size(3)}).mode(torch::kBilinear).align_corners(false));
                pair = torch::cat({lr, sr, hr}, /*dim=*/0);
        ...
        if (vm["valid"].as<bool>() && ((epoch - 1) % vm["valid_freq"].as<size_t>() == 0)){
            valid(vm, valid_dataloader, device, criterion_MSE, criterion_GAN, criterion_Con, gen, dis, vgg, epoch, valid_loss, valid_loss_gen, valid_loss_dis);
        }
