...
#include "networks.hpp"                // ESRGAN_Generator
...
void test(po::variables_map &vm, torch::Device &device, ESRGAN_Generator &gen, std::vector<transforms_Compose> &transformLR, std::vector<transforms_Compose> &transformHR){
    ...
    // (1) Get Test Dataset
    lr_dir = "datasets/" + vm["dataset"].as<std::string>() + '/' + vm["test_lr_dir"].as<std::string>();
    hr_dir = "datasets/" + vm["dataset"].as<std::string>() + '/' + vm["test_hr_dir"].as<std::string>();
    ...
    while (dataloader(data)){
        ...
        lr = std::get<0>(data).to(device);
        hr = std::get<1>(data).to(device);
        ...
        visualizer::save_image(sr.detach(), fname, /*range=*/output_range, /*cols=*/1, /*padding=*/0);
