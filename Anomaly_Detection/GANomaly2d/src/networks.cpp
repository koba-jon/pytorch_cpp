...
EncoderImpl::EncoderImpl(po::variables_map &vm){
    ...
    DownSampling(this->model, feature*8, feature*8, /*BN=*/true, /*LReLU=*/true);                // {8F,16,16}  ===> {8F,8,8}
    DownSampling(this->model, feature*8, vm["nz"].as<size_t>(), /*BN=*/false, /*LReLU=*/false);  // {8F,8,8}    ===> {Z,4,4}
...
DecoderImpl::DecoderImpl(po::variables_map &vm){
    ...
    UpSampling(this->model, vm["nz"].as<size_t>(), feature*8, /*BN=*/true, /*ReLU=*/true);  // {Z,4,4}     ===> {8F,8,8}
