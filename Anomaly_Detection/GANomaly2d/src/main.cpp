...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, Encoder &enc1, Encoder &enc2, Decoder &dec, GAN_Discriminator &dis, std::vector<transforms_Compose> &transform);
...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("loss_adv", po::value<std::string>()->default_value("l2"), "l1 (mean absolute error), l2 (mean squared error), ssim (structural similarity), etc.")
        ("loss_con", po::value<std::string>()->default_value("l1"), "l1 (mean absolute error), l2 (mean squared error), ssim (structural similarity), etc.")
        ("loss_enc", po::value<std::string>()->default_value("l2"), "l1 (mean absolute error), l2 (mean squared error), ssim (structural similarity), etc.")
        ...
        ("ngef", po::value<size_t>()->default_value(64), "the number of filters in convolution layer closest to image in encoder")
        ("ngdf", po::value<size_t>()->default_value(64), "the number of filters in convolution layer closest to image in decoder")
...
int main(int argc, const char *argv[]){
    ...
    // (5) Define Network
    Encoder enc1(vm); enc1->to(device);
    Encoder enc2(vm); enc2->to(device);
    Decoder dec(vm); dec->to(device);
