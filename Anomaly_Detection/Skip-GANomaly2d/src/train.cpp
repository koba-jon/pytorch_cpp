...
// Training Function
// -------------------
void train(po::variables_map &vm, torch::Device &device, UNet_Generator &gen, GAN_Discriminator &dis, std::vector<transforms_Compose> &transform){
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++){
        ...
        show_progress = new progress::display(/*count_max_=*/total_iter, /*epoch=*/{epoch, total_epoch}, /*loss_=*/{"adv", "con", "lat", "D_Real", "D_Fake"});
        ...
        while (dataloader(mini_batch)){
            ...
            // -----------------------------------
            show_progress->increment(/*loss_value=*/{adv_loss.item<float>(), con_loss.item<float>(), lat_loss.item<float>(), dis_real_loss.item<float>(), dis_fake_loss.item<float>()});
        ...
        if (vm["valid"].as<bool>() && ((epoch - 1) % vm["valid_freq"].as<size_t>() == 0)){
            valid(vm, valid_dataloader, device, criterion_adv, criterion_con, criterion_lat, gen, dis, epoch, valid_loss_rec, valid_loss_lat, valid_loss_gan, valid_loss_dis);
        }
