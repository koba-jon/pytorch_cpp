...
GAN_EncoderImpl::GAN_EncoderImpl(po::variables_map &vm){
    ...
    DownSampling(this->model, vm["nc"].as<size_t>(), feature, /*BN=*/false, /*LReLU=*/true);     // {C,256,256} ===> {F,128,128}
    DownSampling(this->model, feature, feature*2, /*BN=*/true, /*LReLU=*/true);                  // {F,128,128} ===> {2F,64,64}
    DownSampling(this->model, feature*2, feature*4, /*BN=*/true, /*LReLU=*/true);                // {2F,64,64}  ===> {4F,32,32}
    DownSampling(this->model, feature*4, feature*8, /*BN=*/true, /*LReLU=*/true);                // {4F,32,32}  ===> {8F,16,16}
    for (size_t i = 0; i < num_downs - 5; i++){
        DownSampling(this->model, feature*8, feature*8, /*BN=*/true, /*LReLU=*/true);            // {8F,16,16}    ===> {8F,2,2}
    }
    DownSampling(this->model, feature*8, vm["nz"].as<size_t>(), /*BN=*/false, /*LReLU=*/false);  // {8F,2,2}    ===> {Z,1,1}
...
GAN_DiscriminatorImpl::GAN_DiscriminatorImpl(po::variables_map &vm){
    ...
    DownSampling(this->x_down, vm["nc"].as<size_t>(), feature, /*BN=*/false, /*LReLU=*/true);  // {C,256,256} ===> {F,128,128}
    DownSampling(this->x_down, feature, feature*2, /*BN=*/true, /*LReLU=*/true);               // {F,128,128} ===> {2F,64,64}
    DownSampling(this->x_down, feature*2, feature*4, /*BN=*/true, /*LReLU=*/true);             // {2F,64,64}  ===> {4F,32,32}
    DownSampling(this->x_down, feature*4, feature*8, /*BN=*/true, /*LReLU=*/true);             // {4F,32,32}  ===> {8F,16,16}
    for (size_t i = 0; i < num_downs - 5; i++){
        DownSampling(this->x_down, feature*8, feature*8, /*BN=*/true, /*LReLU=*/true);         // {8F,16,16}  ===> {8F,2,2}
    }
