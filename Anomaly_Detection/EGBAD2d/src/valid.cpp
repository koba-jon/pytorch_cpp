...
void valid(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device, Loss &criterion, GAN_Encoder &enc, GAN_Generator &gen, GAN_Discriminator &dis, const size_t epoch, visualizer::graph &writer, visualizer::graph &writer_dis){
    ...
    dis->eval();
    iteration = 0;
    total_enc_loss = 0.0; total_gen_loss = 0.0; total_dis_real_loss = 0.0; total_dis_fake_loss = 0.0;
    ...
    z_input = torch::full({1, (long int)vm["nz"].as<size_t>()}, /*value=*/-vm["valid_sigma_max"].as<float>(), torch::TensorOptions().dtype(torch::kFloat)).to(device);
    ...
    for (size_t i = 1; i < max_counter; i++){
        ...
        z_input = torch::full({1, (long int)vm["nz"].as<size_t>()}, /*value=*/value, torch::TensorOptions().dtype(torch::kFloat)).to(device);
