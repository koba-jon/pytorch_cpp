...
po::options_description parse_arguments(){
    ...
    args.add_options()
        ...
        ("select_dim", po::value<size_t>()->default_value(550), "the number of reduced dimensions in random dimensionality reduction")
...
int main(int argc, const char *argv[]){
    ...
    // (5) Define Network
    MC_ResNet model(vm); model->to(device);
    SelectIndex select(model->get_total_dim(device), vm["select_dim"].as<size_t>()); select->to(device);
    ...
    Set_Model_Params(vm, select, "SelectIndex");

    // (8.1) Training Phase
    if (vm["train"].as<bool>()){
        Set_Options(vm, argc, argv, args, "train");
        train(vm, device, model, select, transform);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()){
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, model, select, transform, transformGT);
    }
