...
// Function Prototype
std::tuple<torch::Tensor, torch::Tensor> PaDiMScore(torch::Tensor feature, torch::Tensor mean, torch::Tensor covInv_hw);
...
// Test Function
// ---------------
void test(po::variables_map &vm, torch::Device &device, MC_ResNet &model, SelectIndex &select, std::vector<transforms_Compose> &transform, std::vector<transforms_Compose> &transformGT){
    ...
    torch::Tensor mean, cov, cov_hw, covInv_hw, feature;
    ...
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/select.pth"; torch::load(select, path, device);
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/mean.pth"; torch::load(mean, path, device);
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/cov.pth"; torch::load(cov, path, device);
    mean = mean.to(device);
    cov = cov.to(device);
    ...
    cov_hw = cov.permute({2, 0, 1}).contiguous();  // {H*W,D,D}
    covInv_hw = torch::linalg_inv(cov_hw);  // {H*W,D,D}
    ...
    while (dataloaderN(dataN)){
        ...
        std::tie(score_map, anomaly_score) = PaDiMScore(feature, mean, covInv_hw);
...
std::tuple<torch::Tensor, torch::Tensor> PaDiMScore(torch::Tensor feature, torch::Tensor mean, torch::Tensor covInv_hw){
    ...
    torch::Tensor x, diff, diff_hw_d, a, c, m, dist, score_map, image_score;
    ...
    x = feature.view({1, dim, feature_size}).squeeze(0);  // {D,H*W}
    diff = x - mean;  // {D,H*W}
    diff_hw_d = diff.permute({1, 0}).contiguous();  // {H*W,D}
    a = diff_hw_d.unsqueeze(1);  // {H*W,1,D}
    c = diff_hw_d.unsqueeze(2);  // {H*W,D,1}
    m = torch::bmm(a, covInv_hw);  // {H*W,1,D}
    dist = torch::bmm(m, c).view({-1});  // {H*W}
    dist = dist.sqrt();  // {H*W}
    score_map = dist.view({1, 1, height, width});  // {1,1,H,W}
    image_score = dist.max();
